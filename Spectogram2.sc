...
Spectrogram2 {
	...
	*new { arg parent, bounds, bufSize, color, background, lowfreq=0, highfreq=inf;
	...
	initSpectrogram { arg parent, boundsarg, bufSizearg, col, bg, lowfreqarg, highfreqarg;
		...
		fftDataArrayList = nil!boundsarg.width;
						...
						{ arg buf;
							...
							};
							{
								if(index < imgWidth){
									...
									//fftDataArrayList.postln;
									//res = fftDataArrayList.flop.flat.as(Int32Array);
									//image.setPixels(res, Rect(0, 0, imgWidth, (tobin - frombin + 1)));
									fftDataArrayList.do { |arr, ix|
										image.setPixels(arr, Rect(ix, 0, 1, (tobin - frombin + 1)));
									}
								};
