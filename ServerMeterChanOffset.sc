ServerMeterViewChanOffset {

	classvar serverMeterViews, updateFreq = 10, dBLow = -80, meterWidth = 15, gapWidth = 4, <height = 230;
	classvar serverCleanupFuncs;

	var <view;
	var inresp, outresp, synthFunc, responderFunc, server, numIns, numOuts, inmeters, outmeters, startResponderFunc, inOffset, outOffset;

	*new { |aserver, parent, leftUp, numIns, numOuts, inOffset, outOffset|
		^super.new.init(aserver, parent, leftUp, numIns, numOuts, inOffset, outOffset)
	}
	...
	*getWidth { arg numIns, numOuts, server;
	...
	init { arg aserver, parent, leftUp, anumIns, anumOuts, ainOffset, aoutOffset;
		...
		inOffset = ainOffset ?? { 0 };
		outOffset = aoutOffset ?? { 0 };
			...
			server.bind( {
				...
				if(numIns > 0, {
					insynth = SynthDef(server.name ++ "InputLevels", {
						var in = In.ar(NumOutputBuses.ir + inOffset, numIns);
				...
				if(numOuts > 0, {
					outsynth = SynthDef(server.name ++ "OutputLevels", {
						var in = In.ar(outOffset, numOuts);
...
ServerMeterChanOffset {
	...
	*new { |server, numIns, numOuts|
		...
		window = Window.new(server.name ++ " levels (dBFS)",
			Rect(5, 305, ServerMeterViewChanOffset.getWidth(numIns, numOuts), ServerMeterViewChanOffset.height),
			false);

		meterView = ServerMeterViewChanOffset(server, window, 0@0, numIns, numOuts);
